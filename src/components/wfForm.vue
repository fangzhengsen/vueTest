  ...
  <div class="wf-form-wrap" ref="wfFormWrap">
    ...
    <div class="table-search" ref="tableSearch">
      <el-form ref="tableSearchForm" class="table-search-form">
        <el-row :gutter="20">
          <template v-for="(searchItem, ind) in mergeOption.tableSearch">
            <el-col :xs="24" :sm="12" :md="8" :lg="{ span: (24 / mergeOption.tsRowNum) * (searchItem.column || 1) }"
              v-if="open || !open && !searchItem.shrink">
              <el-form-item :prop="searchItem.prop" :label="searchItem.label" :rules="searchItem.rules">
          ...
          <el-col class="btn-group" :span="24 / mergeOption.tsRowNum">
            ...
            <el-form-item>
              ...
              <el-button v-if="mergeOption.toggleOpen" @click="$emit('toggleOpen', open)" type="text" class="toggle">{{
                open ? "收起" :
                "展开"
              }}
    ...
    <div class="wf-form-content">
      ...
      <el-row ref="menuTop" :gutter="10">
        <el-col class="menu-left" :span="12">
          <div class="t-title" v-if="mergeOption.title">
            {{ mergeOption.title }}
          </div>
        ...
        <el-col :span="12" class="menu-right">
          <el-button type="primary" size="small" @click="showFormDialog('row-save')" v-if="mergeOption.addBtn">{{
            mergeOption.addText
          }}</el-button>
          <!-- 表格头部右侧插槽 -->
      ...
      <div v-show="showCard == 'table'">
        ...
        <!-- 表格式 -->
        <el-table v-else ref="multipleTable" :data="data" size="small" tooltip-effect="dark" style="width: 100%">

          <el-table-column v-if="mergeOption.selection" type="selection" width="55" align="center">
          </el-table-column>
          <el-table-column v-if="mergeOption.index" label="序号" :index="indexMethod" type="index"
            :width="mergeOption.indexWidth || 50">
          </el-table-column>
          <template v-for="(item, index) in mergeOption.tableColumn">
            <el-table-column v-if="!item.hidden" :label="item.label" :key="index" :width="item.labelWidth"
              ...
              <template #default="scope">
                ...
                <span v-else-if="item.type == 'select'" @click="item.callback && item.callback(scope.row, item.prop)"
                  v-html="(item.formate && item.formate(scope.row, item.prop)) ||
                    (getSelectVal[item.prop] &&
                      getSelectVal[item.prop][scope.row[item.prop]])"></span>
                <span class="column-formate" v-else-if="item.formate"
                  @click="item.callback && item.callback(scope.row, item.prop)"
                  v-html="item.formate(scope.row, item.prop)"></span>
                <span @click="item.callback && item.callback(scope.row, item.prop)" v-else>{{ scope.row[item.prop]
                }}</span>
              </template>
            </el-table-column>
          </template>

          <el-table-column label="操作" align="center" v-if="mergeOption.controlWidth > 0"
            class-name="small-padding fixed-width" :width="mergeOption.controlWidth" fixed="right">
            <template #default="scope">
              <slot name="tableBtn" :row="scope.row"></slot>
              <el-button size="small" :text ="true" :disabled="scope.row.viewDisabled" v-if="mergeOption.viewBtn"
                @click="showFormDialog('detail', scope)">{{
                  mergeOption.viewText
                }}</el-button>
              <el-button size="small" :text ="true" :disabled="scope.row.editDisabled" v-if="mergeOption.editBtn"
                @click="showFormDialog('row-update', scope)">
                {{ mergeOption.editText }}</el-button>
              <el-button size="small" :text ="true" :disabled="scope.row.delDisabled" v-if="mergeOption.delBtn"
      ...
      <div v-show="showCard == 'card'" v-loading="tLoading" class="card-content" ref="cardContent">
        <template v-if="data.length > 0">
          <el-row class="card-inner">
            <el-col :xs="24" :sm="12" :md="8" :lg="{ span: Math.ceil(24 / mergeCardConfig.num) * (item.column || 1) }"
              v-for="(item, index) in data" :key="index">
              <div class="card-item" :style="{height: mergeCardConfig.height+'px'}" :ref="'cardDom'+index">
                <!-- 卡片组件插槽 -->
    ...
    <!-- 分页 -->
    <el-pagination v-if="pages" style="margin-top: 10px; text-align: right" ref="tablePage"
      :popper-class="mergePages.paginationClass" @current-change="currentChange" @size-change="sizeChange"
      :pager-count="mergePages.pagerCount"
      :page-sizes="mergePages.sizes" background :current-page="mergePages.current" :layout="mergePages.layout"
      :page-size="mergePages.size" :total="mergePages.total">
    </el-pagination>
    <!-- 弹窗 -->
    <el-dialog class="wf-form-dialog" :top="mergeOption.dialogTop || '15vh'" :custom-class="mergeOption.dialogClass"
      :title="dialogTitle" v-if="dialogFormVisible" v-model="dialogFormVisible"
      :modal-append-to-body="mergeOption.dialogAppend" :append-to-body="mergeOption.dialogAppend">
      <el-form v-loading="mergeOption.dialogLoading" :model="dialogFormData" size="small" ref="dialogFormsRef"
        :label-width="mergeOption.dialogLableWidth || '100px'">
        <template v-for="(item, index) in dialogColumn">
          <el-form-item v-if="item.type != 'slot' && !item.hidden" :prop="item.prop" :label="item.label" :rules="item.rules">
            <switchItem :item="item"></switchItem>
          </el-form-item>
          <template v-else>
            <el-form-item v-show="!item.hidden" :class="['slot-form', item.className]" :key="item.prop + item.type" :prop="item.prop"
              :label="item.label">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" size="small" v-show="dialogType != 'detail'" @click="comfirmDialog"
          :loading="confirmLoading">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
/***事件派发***/
const emit = defineEmits(["update:cardConfig","update:pages","update:search","showDialog","row-save","onLoad","searchReset","searchChange","rowDel","row-update"])
...
/***watch***/
watch(()=>mergeOption.value.tableSearch,(newV,oldV)=>{
  let obj={}
  newV.map((item, index) => {
    obj[item.prop] = item.value;
  })
...
function showFormDialog(type,scope){
  ...
  dialogColumn.value.map((item) => {
    item.value = defaultValueObj.value[item.prop] && JSON.parse(JSON.stringify(defaultValueObj.value[item.prop])) || '';
    item.disabled =
  ...
  if (type == "row-update" || type == "detail") {
    dialogColumn.value.map((that) => {
      for (const key in item) {
        if (key == that.prop && item['dialog'+key]){ // 如果弹窗展示跟列表展示不一样，showDialog处理时增加dialog属性
...
onMounted(() => {
  ...
  // 处理新增默认回显值
  originOption.value.dialogColumn && originOption.value.dialogColumn.map(item=>{
    defaultValueObj.value[item.prop] = item.value&&JSON.parse(JSON.stringify(item.value))
  })
  emit("onLoad");
  calcItemWidth()
})
